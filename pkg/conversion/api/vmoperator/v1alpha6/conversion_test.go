...
func TestFuzzyConversion(t *testing.T) {
	...
	t.Run("for VirtualMachine", conversiontest.RoundTripTest(conversiontest.RoundTripTestInput{
		...
		CheckTypes: conversiontest.RoundTripCheckTypesInput{
			// In v1alpha6, volume fields like ApplicationType, ControllerBusNumber,
			// ControllerType, DiskMode, SharingMode, UnitNumber moved from
			// PersistentVolumeClaimVolumeSource to VirtualMachineVolume.
			// The type checker cannot handle this structural change.
			Skip: true,
		},
	}))
...
func hubPersistentVolumeClaimVolumeSource(in *vmoprvhub.PersistentVolumeClaimVolumeSource, c randfill.Continue) {
	c.FillNoCustom(in)
	// UnmanagedVolumeClaim exists in hub but not in v1alpha6.
